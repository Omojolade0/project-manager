...
def get_note(project_id: uuid.UUID, notes_id: uuid.UUID,
              current_user: User = Depends(get_current_user),
              session: Session = Depends(get_session)):
...
def create_note(project_id: uuid.UUID, data: NoteCreate,
                current_user: User = Depends(get_current_user),
                session: Session = Depends(get_session)):
  return service.create_note(project_id, data, user_id = current_user.id, session=session)

@router.delete("/{note_id}", status_code=204)
def delete_note(project_id: uuid.UUID, note_id: uuid.UUID,
                 current_user: User = Depends(get_current_user),
                 session: Session = Depends(get_session)):
...
def update_note(project_id: uuid.UUID, note_id: uuid.UUID, data: NoteUpdate,
                current_user: User = Depends(get_current_user),
                session:Session = Depends(get_session)):
